...
if __name__=='__main__':
    ...
    if args.verbose:
        print("Loading the dlib and OpenFace models took {} seconds.".format(
            time.time() - start))
    actor_root='/home/byx/openface2016/openface/kelly/go100/actors/'
    ...
    img_path='/home/byx/openface2016/openface/kelly/go100/wow/'
    ...
    print("Length of probe_dirout: ",len(probe_dirout))
    for probe in probe_dirout:
    # while len(probe_dirout)>20:
    #     probe=probe_dirout[0]
        ...
        '''
        mean5=np.array([0.001])
        for j in range(len(actor_path)):
            target_des=os.path.join(actor_root,actor_path[j].split('/')[-2])
            target_dirout=os.listdir(target_des)
            target_sample_idx=random.sample(range(len(target_dirout)),10)

            
            total10_dis=0.0
            target_dirout=np.array(target_dirout)
            for target_img in target_dirout[target_sample_idx]:
                target_img_path=os.path.join(target_des,target_img)
                target_flag,target_rep=getRep(target_img_path)

                diff=probe_rep-target_rep
                dis_pt=np.dot(diff,diff)
                total10_dis+=dis_pt
            mean5=np.append(mean5,total10_dis/10)
            '''
            if total10_dis/10<1.0 :
                shutil.move(img_path+probe,target_des)
                probe_dirout=os.listdir(img_path)

                print('************************************************')
                print("Moving {} to {}.".format(probe,target_des))
                print('************************************************')
                break
            else:
                print("The distance between {} and {}.".format(probe,actor_path[j].split('/')[-2]))
                print("  + The mean distance is {}.".format(total10_dis/10))
            '''

        final_target_idx=np.argmin(mean5[1:])
        actor_des=actor_path[final_target_idx].split('/')[-2]
        final_target_des=os.path.join(actor_root,actor_des)
        shutil.move(img_path+probe,final_target_des)
        print('************************************************')
        print("Moving {} to {}.".format(probe,final_target_des))
        print('************************************************')
