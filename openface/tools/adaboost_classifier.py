...
def train(args):
    ...
    param_grid = [
        {'n_estimators': [25,50,200,500,1000],'learning_rate': [0.01,0.05,0.1,0.15,0.2]}
    ]
    adaboost = GridSearchCV(
        AdaBoostClassifier( DecisionTreeClassifier(max_depth=4)),
        param_grid, verbose=2, cv=5, n_jobs=16
    ).fit(embeddings, labelsNum)
    print("Best estimator: {}".format(adaboost.best_estimator_))
    print("Best score on left out data: {:.2f}".format(adaboost.best_score_))

    with open("{}/adaboost_classifier.pkl".format(args.workDir), 'w') as f:
        pickle.dump((le, adaboost), f)


def infer(imgpath):
    with open(args.classifierModel, 'r') as f:
        (le, adaboost) = pickle.load(f)
    rep = getRep(imgpath)
    predictions = adaboost.predict_proba(rep)[0]
    #print predictions
